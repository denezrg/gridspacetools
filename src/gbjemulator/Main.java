...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        //outputGridspace(gridspace);
        gridspace = setPos(gridspace, 0, 0, 3);
        //outputGridspace(gridspace);
        gridspace = propagateField(gridspace, 1);
        outputGridspace(gridspace);
    }
    ...
    static float[][] propagateField(float[][] igsp, int cycles){
        ...
        for(int c = 0; c <= cycles; c++) {
            ...
            for(int j = 0; j < h-1; j++) { // searching height, j = current y 

                for(int i = 0; i < w-1; i++) { // searching width, i = current x 
